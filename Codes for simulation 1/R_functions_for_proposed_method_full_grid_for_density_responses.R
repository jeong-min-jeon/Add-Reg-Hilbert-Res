# R functions for proposed method with full grid for density response 
...
# Function for Full grid bandwidths selection
...
# seq(min_h[j],min_h[j]+h_add[j],length=h_length[j]) will be the candidate set of j-th bandwidth
# for some small bandwidth min_h[j]
Full_grid_density=function(X_training,Y_training,add=11,Y_grid=NULL,max_iteration=50,epsilon=10^-4
                     ,cv=10,h_add=NULL,h_length=NULL)
{
  n=nrow(X_training)
  d=ncol(X_training)
  T=ncol(Y_training)
  ...
  H_matrix=as.matrix(expand.grid(as.data.frame(h_candidate_matrix)))
  H_matrix=H_matrix[complete.cases(H_matrix),]
  ngrid_h=nrow(H_matrix)
  
  result=.Fortran('Full_grid_continuous_density',n=n,d=d,T=T,X_training=X_training,Y_training=Y_training,
                  H_matrix=H_matrix,add=add,max_iteration=max_iteration,epsilon=epsilon,time_vector=Y_grid,
                  ngrid_h=ngrid_h,cv=cv,cv_test_size=cv_test_size,
                  optimal_h=rep(0,d),total_iteration=array(as.integer(0),c(ngrid_h,cv)))
  return(list(optimal_h=result$optimal_h,SBF_iteration=result$total_iteration))
}
...
SBF_density=function(X_test,X_training,Y_training,add=11,Y_grid=NULL,max_iteration=30,epsilon=10^-4,h)
{
  n=nrow(X_training)
  d=ncol(X_training)
  T=ncol(Y_training)
  ...
  result=.Fortran('SBF_continuous_density',n0=N,n=n,d=d,ngrid=ngrid,T=T,X0=X_test,X=X_training,Y=Y_training,g=g,h=h,
                  max_iteration=max_iteration,epsilon=epsilon,time_vector=Y_grid,actual_iteration=actual_iteration,
                  yhat=yhat,mhat=mhat)
