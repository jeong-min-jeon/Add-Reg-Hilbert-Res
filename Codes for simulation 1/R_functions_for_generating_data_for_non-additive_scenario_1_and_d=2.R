# R functions for generating data for non-additive scenario I and d=2

# Load necessary package
library(pracma)

f12=function(x1,x2,t)
{
  exp(-x1*t)*exp(-2*x2^2*t^2)*exp(-x1*x2*t^2)
}
...
{
  temp=f12(x1,x2,time_vector)*f_error(e,time_vector)
...
data_make=function(M,n,d,N,error_range,time_vector)
{
  ...
  return(list(X=X,Y=Y,Y_true=Y_true,test_X=test_X,test_Y=test_Y))
}
