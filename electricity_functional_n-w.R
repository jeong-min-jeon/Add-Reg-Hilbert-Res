...
optimal_h=function(X,Y,time_vector,nfolds,h_add,h_length)
{
  ...
  for(j in 1:h_length)
  {
    for(k in 1:nfolds)
    {
      ...
      Y.test.hat=predict_n-w(matrix(X.test,ncol=d),X.training,Y.training,h_vector[j])
...
for(i in 1:n)
{
  ...
  Y_hat[i,]=predict_n-w(matrix(X[i,],ncol=d),X[-i,],Y[-i,],h_selected[i])
