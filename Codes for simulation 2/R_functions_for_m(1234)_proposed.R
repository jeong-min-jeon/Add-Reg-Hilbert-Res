# R functions for 4-dimensional propsed method for table 2
...
simulation_proposed=function(X,Y,time_vector,h_length,h_add)
{
  ...
  for(k in 1:d)
  {
    for(t in 1:T)
    {
      ...
      if(k==3)  m_true[k,,t]=f3(grid,time_vector[t],time_vector)
      if(k==4)  m_true[k,,t]=f4(grid,time_vector[t],time_vector)
    }
  }
  ...
  for(j in 1:M)
  {
    ...
    m_hat[j,,,]=SBF_density(grid_matrix,X[j,,],Y[j,,],h=optimal_h,add=0,Y_grid=time_vector)$m_hat
  }
...
result=simulation_proposed(data$X,data$Y,seq(-0.5,0.5,length=101),h_length=rep(21,4),h_add=rep(0.2,4))
