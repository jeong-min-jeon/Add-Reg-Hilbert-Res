# R functions for m(1,2) propsed method for table 2
...
simulation_proposed=function(X,Y,time_vector,h_length,h_add)
{
  ...
  for(k in 1:d)
  {
    for(t in 1:T)
    {
      if(k==1)  m_true[k,,t]=f1(grid,time_vector[t],time_vector)
      if(k==2)  m_true[k,,t]=f2(grid,time_vector[t],time_vector)
    }
  }
  
  # Obtaining oracle information for the 3rd and 4th component maps
  m_true_data=array(,dim=c(4,M,n,T))
  for(k in 3:4)
  {
    for(j in 1:M)
    {
      for(t in 1:T)
      {
        if(k==3)  m_true_data[k,j,,t]=f3(X[j,,3],time_vector[t],time_vector)
        if(k==4)  m_true_data[k,j,,t]=f4(X[j,,4],time_vector[t],time_vector)
      }
    }
  }
  ...
  for(j in 1:M)
  {
    for(i in 1:n)
    {
      temporary=Y[j,i,]/m_true_data[3,j,i,]/m_true_data[4,j,i,]
  ...
  {
    print(j)
    optimal_h=CBS_density(X[j,,1:2],Y[j,,],add=2,Y_grid=time_vector,h_length=h_length,h_add=h_add)$optimal_h
    m_hat[j,,,]=SBF_density(grid_matrix,X[j,,1:2],Y[j,,],h=optimal_h,add=0,Y_grid=time_vector)$m_hat
  }
