# R functions for proposed method for functional response 
...
# seq(min_h[j],min_h[j]+h_add[j],length=h_length[j]) will be the candidate set of j-th bandwidth
# for some small bandwidth min_h[j]
CBS_L2=function(X_training,Y_training,add=11,Y_grid=NULL,max_iteration=50,epsilon=10^-4
...
SBF_L2=function(X_test,X_training,Y_training,add=11,Y_grid=NULL,max_iteration=30,epsilon=10^-4,h)
{
  n=nrow(X_training)
  d=ncol(X_training)
  T=ncol(Y_training)
