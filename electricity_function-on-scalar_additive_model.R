# Code for function-on-scalar additive model by Scheipl et al. (2015)
...
for(i in 1:n)
{
  ...
  fit=pffr(Y~s(X1)+s(X2),data=data_training,yind=eval_vector,bs.yindex=list(bs='ps',k=k,m=c(2,1)),bs.int=list(bs='ps',k=k,m=c(2,1)))
